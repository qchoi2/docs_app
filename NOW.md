...
| 2 | T-D backfill + 재분류 — **두 단계를 반드시 함께** | `backfill_v4_candidate_recurrence.py` → `reclassify_v4_candidate_backlog.py` | ◑ **backfill 적용됨**(document_count 31,394행). **reclassify는 `--apply` 대기** — 자동 분류기가 조율자 self-grant를 차단하므로 **소유자가 `python3 reclassify_v4_candidate_backlog.py --out cs_index --apply` 실행 필요**. dry-run 검토 완료: pending 29,807→13,222(one-off 16,585 은퇴, 사람결정·item 보존) |
...
| 22 | **랭킹 품질** — nDCG@10 + 상위 노출 개선 | 미배정 | **우선순위 상향(실측 근거)** | ~~후반 과제~~ — 절대 recall 실측이 **중앙 순위 462위 · recall@10 1.5%**를 보였다. 저장된 정답지의 69.2%를 **"찾기는 하는데 사람이 볼 위치에 없다"** — 현행 최대 병목 | [V4_PLAN §9.6](.docs/V4_PLAN.md) |
