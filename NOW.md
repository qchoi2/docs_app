...
| 22 | **랭킹 품질** — 질의 신호 강화(완료)·나머지 T4 | 부분완료→T4(V4-7) | **진단 확정·에이전트 개선 착수(2026-07-30)** | 462위·1.5%는 **질의 신호 부재**(개념 질의가 노드 2.9k~18k 무순위 열거). 3단 구도 실측(`eval_ranking_signal.py`): 개념 1.5% / **패러프레이즈 59.6%(T4 표적·사전등록 베이스라인, verbatim커버리지 0.415)** / verbatim 92.7%. 착수: **#1 다중키워드 text**(토큰 AND) · text 관련도정렬(변별 recall@1 0.536→0.623) · **#5 MCP docstring**(text=랭킹신호) · **#6 low_query_signal 힌트** · **#7 v4_query_log**(T4 실데이터). 487 pass. 보류: #4 정적prior·#8 사전확장·#9 하이브리드/dedup실측·#10 다양화 | [V4_PLAN §9.6](.docs/V4_PLAN.md) |
