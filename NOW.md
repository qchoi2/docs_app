...
| 22 | **랭킹 품질** — FTS+bm25 전환 완료·나머지 T4 | 부분완료→T4(V4-7) | **진단 확정·FTS 배선·공정 베이스라인(2026-07-30)** | 462위·1.5%는 **질의 신호 부재**. **최대 발견: `v4_item_fts`(137,865 동기)가 미배선**이었음 → text 경로를 FTS MATCH+bm25 하이브리드로 전환(연속구문 우선→bm25 IDF, <3자 LIKE 폴백). verbatim recall@10 **0.90** 유지, paraphrase **0.633**(공정 T4 베이스라인, 조사제거+bm25). 착수: #1 다중키워드·#5 MCP docstring(+하이브리드 지침)·#6 low_query_signal·#7 v4_query_log·0건힌트·coverage IN-filter. 488 pass. **T4 선행조건 승격**: #9(a) 하이브리드 합집합 실측(#17). 보류: #4 정적prior·#8 교차언어 사전확장·#10 다양화 | [V4_PLAN §9.6](.docs/V4_PLAN.md) |
