...
| 6 | **존재형·비교형 정밀도 ≥90% 측정** — family별(RW·COV·PAY) 사람 원문대조 표본 + COV.NON_COMPETE 오분류 수정 재측정 | 미배정 (표본 검증은 소유자) | **COV 부분 진척(2026-07-30)** — 오분류 138건 RW.CONTRACTS 이동 완료(425→287), 분류기 규칙 정련(매수측 주어·경업금지 frame → keep 35→45). 남은 버킷 review 181·noise 61. **분류기 proxy는 하한일 뿐 — 게이트 통과는 소유자 원문대조 표본 필요**(미착수). noise 61 삭제는 소유자 승인 대기 | [V4_PLAN §9.1 #5·§9.3](.docs/V4_PLAN.md), `lib/classification_audit.py` |
...
| 17 | **절대 recall 측정** — `eval_absolute_recall.py` | 조율자 | **1차 완료**(structured). 하이브리드 합집합 미측정 → **T4 선행조건으로 승격**(§9.6-b #9a: V4-7 ablation의 비교대상이라 필수) | 풀링은 **상대** 재현율만 준다 — 둘 다 못 찾은 문서는 영원히 안 보임. 정독 부산물로 정답지가 **이미 공짜로 쌓여 있다**. 이 프로젝트 최초의 절대 recall 수치 | [V4_PLAN §9.5](.docs/V4_PLAN.md) |
...
| 22 | **랭킹 품질** — FTS+bm25 전환 완료·나머지 T4 | 부분완료→T4(V4-7) | **진단 확정·FTS 배선·공정 베이스라인(2026-07-30)** | 462위·1.5%는 **질의 신호 부재**. **최대 발견: `v4_item_fts`(137,865 동기)가 미배선**이었음 → text 경로를 FTS MATCH+bm25 하이브리드로 전환(연속구문 우선→bm25 IDF, <3자 LIKE 폴백). verbatim recall@10 **0.90** 유지(@1 미스는 정형문구 동일반복→채점 아티팩트, rank=None 0건으로 실측), paraphrase **0.633**(공정 베이스라인, 단발; **ablation은 어휘+재질의루프 vs T4+루프**로 비교). 착수: #1 다중키워드·#5 MCP docstring(+하이브리드 지침)·#6 low_query_signal·#7 v4_query_log·0건힌트·coverage IN-filter. 488 pass. **T4 선행조건 승격**: #9(a) 하이브리드 합집합 실측(#17). 보류: #4 정적prior·#8 교차언어 사전확장·#10 다양화·**#2 개념-only 열거 윈도우함수 dedup(백로그)** | [V4_PLAN §9.6](.docs/V4_PLAN.md) |
