...
### 2026-07-29 — Fable 2차 지적 반영: 검증 축 3개 신설 + 사각지대·메타데이터 실측 (Claude, opus 5 + 서브에이전트 3)

**소유자 일괄 승인(2026-07-29)**: 이 라운드에서 제기된 미결정 사항은 **전부 권고안대로 채택**.

- **② 코퍼스 사각지대(실측)** — `.docs/CORPUS_BLIND_SPOTS_20260729.md`. "48건"은 맞지만 과소:
  실제 사각지대는 **90파일 / 논리문서 56건**(empty 48 + unsupported 41 + error 1; jpg 34장은
  스캔 ATA/BTA 1건). PDF 내부 확인 결과 **47건이 진짜 스캔**(폰트 객체 0, Producer가 복합기 모델),
  암호화·0바이트·파서실패 0, **검색가능 문서의 중복본 0 = 순수 손실**. 총 1,931쪽. 급소는 **SHA 10건**
  (SHA 커버리지 2.1%, 2025 체결본 포함).
  - **전제 2개 반증**: (a) "읽을 수 없는 별지" 가설 기각 — `v4_source_coverage` missing 502행(148문서)은
    전부 `storage_file_key` NULL이고 비정상 파일을 가리키는 행 0 → **애초에 수집 안 된 것**,
    OCR로 복구 불가(소싱 문제로 분리). (b) `.doc` 변환은 **이미 완주**했고 잔여 5건은 실패가 아니라
    **확장자만 .doc인 RTF**(`detect_doc_kind()`가 Word 전에 거부) → 가장 값싼 recall 회수.
  - **OCR: 새 의존성 0·₩0·약 1시간**(`Windows.Media.Ocr`에 `ko` 기설치 + `Windows.Data.Pdf`).
    실제 코퍼스 2건 시연: 조문 제목·금액은 정확. 단 **항목 번호가 본문에서 분리·재배열**되고
    글자 오인이 의미를 뒤집음(`부채`→`부재`). → **승인하되 OCR 텍스트는 `confirmed_absent` 불가
    (항상 needs_review, 사유 `ocr_derived_text`) · V4 추출 제외(FTS 전용)**.
  - 잠재 함정: 90파일이 `content_hash=e3b0c442…`(빈 문자열 SHA) 공유 — 추출 실패가 전부 충돌.
    **현재 `dup_group`은 분리돼 있어 실害 없음**(향후 dedup 코드의 지뢰). dedup 키를 원본 바이트
    `source_sha256`로 이전 승인.
- **③ 비교형 셀 정확도 축 신설(§9.4)** — §0이 비교·집계를 **"필수"**로 분류해놓고 정확도 검증 계획만
  없던 공백. 비교형은 **셀 단위**로 틀린다(빈 셀이 "조항 없음"으로 읽힘 / 다른 층위 조항을 같은 행에 /
  단위·기준일 달라 비교 불가 / verbatim은 맞고 조항 선택이 틀림). **빈 셀은 자동 정답이 아니다** —
  미평가 표시가 있어야 correct, 그냥 비면 `false_empty`. **확정: 셀 정확도 ≥90%, V4C01~06 전수.**
- **④ 절대 recall 축 신설(§9.5) + 1차 실측** — `eval_absolute_recall.py`. 정답지 = full_read 문서
  **465 (문서,family) 키 / 388 파일 / 14,433 item**. structured 경로 깊이 2000 이내 **47.8%**.
  - **헤드라인을 검색 품질로 인용 금지** — 정답지 465 중 **171(4,665 item)이 아직 store 전**이라
    2.8%이고(=DB에 없으면 못 찾음, 당연), **저장분만 보면 69.2%**. 둘의 혼합값이다.
  - **진짜 발견은 랭킹**: **중앙 순위 462위 · recall@10 = 1.5%**. 찾기는 하는데 **사람이 볼 위치에
    없다.** → §9.6 랭킹 품질이 후반 과제가 아니라 **실측된 현행 최대 병목**으로 격상.
  - 유형별: SSA 92.0% · SHA 74.7% · CB인수 60.5% vs **SPA 44.7%**(정답지 89%가 SPA라 헤드라인 지배).
    family별 RW 63.2% vs PAY 27.7%. **한계: structured 경로만 측정**(하이브리드 합집합 미측정).
- **⑥ DEF 구조화 검색 — 완료 범위 포함 확정**. DEF item **78%(16,077/20,600)**가 캐치올 흡수라
  정의조항 검색이 사실상 FTS 전용. 도구 기반은 5a579cc에 이미 있고 **기존 item 소급 적용**이 남은 일.
- **⑦ T4 = V4-7 예약 실험 확정**(§10 표 반영). 강등 근거가 비용·복잡도였으므로 품질 최우선 전환 시
  계산이 달라진다. **채택은 골든셋 ablation 결과로만**, 기여도 미측정 시 폐기.
- **⑧ 거래 메타데이터(실측)** — `.docs/DEAL_METADATA_FEASIBILITY_20260729.md`, `derive_deal_meta.py`(53 tests).
  **채택**: 준거법 70.1% · 관할법원 51.3% · 중재합의 95.7% · 거래연도(체결일 420건, 전부 execution).
  **반대**: 규모 구간 · 업종.
  - **제안 전제가 사실과 다름**: "v3 대금이 있으니 규모 구간 파생 가능" → `doc_meta` 1,999행 중
    **v3 스키마는 60행**, 숫자 `amount_value` 17행, MOU·초안·비KRW 제외 후 **밴딩 안전한 건 8건**.
    48개 평가 섹션 중 **31개는 사람이 의도적으로 null 처리**했고, 세션 24~26이 기록한 세 함정
    (별도 SSA의 RCPS 투자금 300억, 원 EB 전자등록총액 800억, 비구속 별첨 500억)이 실제로 전부 발생했다.
    **자동 규칙이 아니라 문서별 사람 정독이 잡아낸 것**이라 자동화 불가.
  - 업종: 1,395문서가 업종 키워드 ≥1개지만 778문서가 ≥2개, "제조" 742건 대부분이 제조물책임
    보일러플레이트. **업종은 계약서 텍스트의 사실이 아니라 대상회사의 사실** → 로컬 파생 불가.
  - **방법론 교훈**: 단순 전문 키워드 탐침이 준거법을 최대 **126배 과대보고**(홍콩 379 vs 실제 3).
    자회사 목록·당사자 주소·"소송" 정의가 중재절차를 열거해 오탐. **조항 위치를 먼저 특정한 뒤 세야 한다.**
- **⑨ 랭킹 품질(§9.6)** — 신설. 위 실측으로 우선순위 상향.
- **⑤ 완성 후 갱신 런북(§10.2)** — **프로젝트 마무리 단계 필수 산출물**로 고정, NOW.md 차단표에 상설 표기.
  정정: 절차가 없지는 않다(`색인 업데이트 설명서.md` 9단계). 다만 (a) "새 환경으로 전달"용 프레이밍이고
  (b) 07-28~29 변경분(재추출·full_read·게이트·후보정책·버전분류)이 빠져 낡았다. 마지막에 **다시 쓴다**.
  담을 4가지: 명령 순서 / 품질 게이트 / 예상 공수 / **부분적용 표시 규칙**(이중 코퍼스 방지).
...
- **§9.6 Fable 2차 검토 반영(질의 신호 강화).** "잔여는 T4" 서술이 부정확했음을 교정: T4는 개념-라벨 벤치마크를
  ...
  실측, #10 문서 다양화.
- **§9.6-b FTS+bm25 전환 + 공정 베이스라인(Fable 3·4차).** 최대 발견: `v4_item_fts`(FTS5 trigram, proposition+verbatim,
  **137,865=137,865 동기·트리거 유지**)가 조회에 **미배선**(text는 LIKE 전체스캔) — 설계-구현 드리프트. text 경로를 FTS
  MATCH+bm25 **하이브리드**로 전환(연속 전체구문 우선→bm25 IDF; <3자 trigram 불가 토큰 LIKE 폴백). bm25 단독은 인용
  경로 회귀(verbatim recall@1 0.63→0.44)라 구문-우선 하이브리드로 복원(verbatim @10 **0.90**, paraphrase **0.633**).
  **공정 T4 베이스라인 확정**: 종전 0.596은 핸디캡(AND 조사 0건화+최장토큰=조사부착) → 조사제거(`strip_josa`)+bm25로
  재측정한 **paraphrase @10 0.633(cov 0.435)**가 V4-7이 이겨야 할 수치. (순수 OR+bm25 천장은 흔한 토큰이 전 코퍼스를
  스코어링해 비현실적—폐기; 이 자체가 AND+bm25 선택 근거.) 부수: 0건 힌트(부재 오독 방지)·narrowed를 text중심으로·
  웹 진입점 로깅·`_bulk_coverage_states` IN-filter(#2/#5). 488 pass. **#9(a) 하이브리드 합집합 실측은 보류→T4 선행조건 승격**
  (ablation 비교대상). #8은 교차언어 사전확장으로 T4 전 분리.
- **§9.6-b Fable 후속 3건 반영.** (1) verbatim @1 하락을 `--dump-misses`로 실측 — top-10밖 12건 **전부 rank>10, rank=None
  0건**(FTS가 항상 검색), 원인은 정형 조항문구가 여러 계약에 동일 반복("No Proceedings…" 덤프 2회)돼 bm25가 사본 못 가름 +
  eval이 N개 동일 item 중 1개만 정답 채점하는 **아티팩트**(랭킹 결함 아님). (2) 0.633은 단발 수치 → **ablation은 어휘+재질의
  루프 vs T4+루프**로 비교해야 공정(프로토콜 명시). (3) text 없는 브라우징의 `show_duplicates=False` 전량 구체화(윈도우함수
  dedup)는 이번 미해결 → **#2 백로그 유지**.
- **§9.1 #5 COV.NON_COMPETE 엄밀 실측·정리(2026-07-30, 소유자 승인).** noise 61 삭제 승인받아 진행 — 61 중 6건은
  실제 콘텐츠(3 공시진술 조각→RW.CONTRACTS 재분류, 3 공개목록 제외사항→보존)라 **55삭제+3재분류+3보존**(백업·전행로그·참조
  가드). 이어 review 181 **AI-정독 60표본**: 정답 33%·공시진술 25%·타family 18%(소송/노무/IP/구제)·헤딩 23% → **노드
  정밀도 ≈51%, ≥90% 크게 미달**. 분류기 3차 확장(매수측 주어·경업금지 frame·중요계약 disclosure rep·TOC 리스트)으로 공시진술
  13 추가 재분류 + TOC 5 삭제. COV.NON_COMPETE **425→211**(누적 헤딩 60삭제·공시진술 151→RW.CONTRACTS). **규칙 정리는
  ~50%에서 정체 — 잔여는 타family 오분류·다양표현이라 재추출(§9.3-3 경계필드)이 답**. classification_audit 12 pass.
- **추출 백로그 측정(2026-07-30).** **#20 DEF 캐치올**: DEF 22,180 중 DEF.CONTRACT_TERM 캐치올 **18,404=83.0%**(개별
  정의용어들이 한국어 canonical 프리픽스 보유하나 노드 미승격 → 정의검색 FTS 전용; 명명 노드 ~15개). 18k 정규화는 별건.
  **#9 후보 backlog 재분류**: dry-run상 pending **13,222**(옛 29,807서 감소) 전부 정당(재발≥2문서 11,255+구체 sub-node
  1,967), 일회성 0 → **적용 moot**(0건). 남은 pending은 실제 taxonomy 제안이라 유지가 맞음.
- **남은 v4**: §9.1 #2(IP·보험·노무·소송 부재풀은 재추출 후·후순위), #5(RW·PAY 표본+소유자 검증; COV·DEF는 재추출 필요),
  §9.6 잔여(#8 교차언어·T4), #16 비교형 셀(소유자 라벨링), #12 경계필드(시점고정). **자동화로 남은 건 대부분 재추출/소유자 게이트**.
- **RW 정독 재추출 추가 40건(2026-07-30, 소유자 지시).** 배치 1·2 각 20건, 문자수 균형 5샤드×2 서브에이전트 정독
  (result JSON만 쓰기, store·DB·git 조율자 단독). 상세: `.docs/RW_REEXTRACT_NEXT40_20260730.md`,
  소유자 검토 원자료 `.docs/rw_reextract_next40_owner_review_20260730.json`.
  **40/40 정독 · 2,645항목(자동 3,025→2,645) · 매수인 진술 251 신규 · 39저장 · 오류0 · 후퇴스킵0 · integrity ok.**
  store가 `--result-dir` 전체(814파일, 약 400이 낡은 자동추출)를 스캔하므로 **이번 40건만 담은 전용 디렉터리로 범위 한정**
  (`cs_index/rw_reextract_next40_20260730/`). 세션 한도로 배치2가 중단됐으나 문서 단위 체크포인트로 유실 0, 잔여 11건 재배정 완료.
- **핵심 발견: 자동추출 결함의 단일 뿌리 = 추출기가 진술 조항 범위를 경계하지 않음.** 세 갈래로 동시 발현.
  (a) **별지 편입형 과소추출** — 본문이 별지를 인용만 하고 실질 진술은 별지에 있는 형태가 국문 계약의 **지배적** 형태
  (150건 배치 §4는 9건으로 봤으나 이번 40건은 과반). `ead5102fdaeb9104` 83항목 중 66이 별지에서, `4997e306e66695ec`
  REAL_ESTATE 10항목이 전부 편입 공개목록에서만. 이중 편입 4건.
  (b) **범위 밖 긁어오기 → false-present** — `167299b34d606e60` 57항목 중 진술 범위 내 **1건**(나머지는 정의조항·확약목록·
  boilerplate·임직원 서식), `e7ce3f8a57347935` 209항목 중 `RW.BUYER` 75건인데 실제 매수인 진술은 4문단.
  (c) **무내용 템플릿 proposition** — 코퍼스 전체 RW 58,318항목 중 **17,255건(29.6%)**, 946문서 중 **390문서(41.2%)**.
  랭킹이 읽을 문서 측 신호가 30% 비어 있어 **T4 baseline 측정 전 분리 필요**.
- **하락 도메인 전건 원문 대조(사후 검토 부담 0).** 항목 총계는 줄었으나 전부 정정: bare RW 18문서(구체 노드 치환=개선),
  `RW.CUSTOMERS_SUPPLIERS` 6문서·`RW.ENVIRONMENT` 4문서 **전건 false-present 확정**, `RW.DISCLOSURE` 6문서(CP 오분류 제거).
  환경 4건 중 3건은 원문 전체 환경 어휘 **0건**, 1건은 정의조항 단일 히트(진술 조항은 별개 범위). 기전 특정 —
  **포괄 법령준수 진술 한 문장을 여러 하위 도메인으로 복제**(¶21 하나가 AUTHORITY/COMPLIANCE/ENVIRONMENT/LITIGATION 4개로).
  고객공급업체 6건은 실사 협조 확약("매수인이 …임직원, **고객** 기타 계약 상대방을 접촉하지 않도록") 등 범위 밖 우연 등장.
- **환경 게이트 재검토 대상 특정(⛳6).** env 게이트는 부재 풀 89건(precision 100%)에 근거했으나 **present 쪽 미검증**.
  스캔 결과 RW.ENVIRONMENT* 보유 573문서 중 유일 노드가 `.COMPLIANCE`인 문서 132, 그중 **원문 환경 어휘 0건이 18건**.
  이번 정독이 잡은 3건이 전부 이 18건 안 — 표본이 아니라 **모집단 특정**. 잔여 15건은 판단 여지 없는 기계적 정정(소유자 승인 대기).
- **다음 배치 표적 제안: RW.BUYER 과다추출 코호트.** 문서당 buyer 항목 중위 6·p90 15인데 **30건 이상이 18문서**(최대 145).
  이번 배치에 든 2건 모두 과다추출 확인(75→6, 55→6) → **표본 2/2 결함, 잔여 16건**. 브리프 표적 우선순위 1번(매수인 진술 누락)의
  **역방향 신호도 동등하게 강한 선별자**임이 데이터로 확인됨.
- **설계 공백(⛳7): 정독으로 확정한 부재를 DB에 기록할 경로가 없다.** `9d650005bcadc556`(SPA 수정계약)은 전문 정독으로
  "자체 진술 조항 없음"을 확정해 `items: []`+`reason`을 남겼으나 store가 `skipped_no_items`로 스킵 — 150건 배치에서도 11건 동일.
  부재 질의가 목표인데 확정 부재가 `present=false`가 아니라 "미평가"로 남는다.
- **[정정] RW_REEXTRACT_150 §4의 "실재하지 않는 leaf 창작"은 사실이 아니다.** `RW.DISCLOSURE.ACCURACY`는 `RW.DISCLOSURE`와
  함께 taxonomy_version 6에 `origin=seed`로 들어온 실재 노드(`status=active`, 승격 이력 0). 기계 검증상 DB의 RW 항목과
  자동추출 result 파일 모두 실재하지 않는 taxonomy_id **0건**. 150건 보고서에 정정 반영. 자동추출 결함은 위 세 갈래로 확정.
  (조율자 오류: 이 잘못된 정보를 배치2 resume 프롬프트에 넣었으나, 에이전트가 도메인 노드를 사용해 결과는 유효·검증 오류 0.)
- **회귀**: `eval_search.py` pass6/fail0 — 회귀 없음. `eval_v4_gate.py --pooled` scored16·pending4·**unjudged 493**
  (legacy relative recall 0.9412 / v4 0.0982) — precision 셀은 소유자 판정 없이 산출 불가, 구조 불변.
  `burndown.py` RW 부재 질의 **가능 0·차단 1,800·family_gated=Y** 불변. **따라서 이 재추출로 RW 부재정밀도가 회복됐다고
  주장할 수 없다** — §9.1 체크리스트 2·5 중 "표본 원문대조" 재료를 40건 추가한 것이 이번 기여다.
