...
## T3 개발(부록 A) 상태 및 A-2 게이트 결정 (2026-07-12)

- **부록 A(A-1~A-5) 완료**: enrich_contracts.py 하네스, read_contract.py, search_contracts
  `--clause/--present/--absent`, eval `--tiers T1,T2,T3`를 파일럿 색인 위에서 구현·테스트 완료
  (progress.md 세션 3~7). 색인 read 경로(T1/T2)에는 영향 없음.
- **A-2 프롬프트 개선 #1~#7 전량 수용 결정(소유자)**: `A2_SAMPLE_QUALITY_20260711.md`의
  오탐 위험·제안을 모두 수용. 반영 프롬프트를 `.docs/extract_prompt_v2.md`
  (meta_schema_version 2)로 작성함. v1은 이력 보존용으로 유지.
- **미반영(다음 코드 작업)**: 하네스가 아직 v1이다. `enrich_contracts.py`의
  `META_SCHEMA_VERSION`이 1이고, 손해배상 하위필드 강제·`present` 필수 검증이 없다.
  이 반영(하네스 v2 + 샘플 재추출)은 **NEXT_STEPS.md 부록 C(C-0/C-1/C-2)** 에 Codex
  프롬프트로 스테이징되어 있으며, 전량 추출 배치 전에 완료해야 한다. 코드+테스트 실행이
  필요해 파일 도구만 가용한 세션에서는 직접 구현하지 않고 프롬프트로 남겼다.
- **golden_queries.yaml의 T3 문항**: 골격은 있으나 `expected_files`가 비어 있어 현재는
  부분채점(partial)만 된다. 정답 file_key를 채우면 T3 recall이 실측된다(소유자 관할).
- **`convert_doc.py`(.doc→.docx, MS Word) 이탈사항**: 계획상 Windows + MS Word 전용
  유틸리티로, PowerShell COM을 subprocess로 호출해 새 파이썬 의존성 없이 구현하기로 함
  (NEXT_STEPS 부록 B). 아직 미구현.
