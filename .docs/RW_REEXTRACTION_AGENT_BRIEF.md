...
**진술 조항을 처음부터 끝까지 직접 정독해 그 문서 RW 전체를 재구성한 결과에는 반드시
result JSON 최상위에 `"review_method": "full_read"`를 넣어라.** 이유:
- 조율자 store에는 후퇴 가드가 있다 — 새 결과가 기존 DB의 RW 하위도메인을 하나라도 떨어뜨리면
  기본적으로 **저장을 스킵**해 옛 결과(과거 자동추출값)를 보존한다. 불완전한 자동 재작성이 실질
  진술을 지우는 것을 막기 위함이다.
- 그런데 **정독 결과는 그 문서 RW 전체를 담은 권위 있는 세트**다. 정독으로 어떤 도메인이 빠졌다면
  그건 (a) 원문에 실제로 그 진술이 없거나 (b) 자동추출이 오분류했던 것을 **정확히 바로잡은** 것이다.
  이때 가드가 정독 결과를 스킵하면 **정확한 수정이 통째로 버려진다**(실제 발생 사례: 0844 문서 —
  정독으로 16→51항목·매수인 1→9로 개선했으나 SOLVENCY 도메인 1개 감소를 이유로 전량 스킵됨).
- `"review_method": "full_read"`가 있으면 store가 **그 문서 한정으로 가드를 해제**해 정독 세트를
  그대로 저장하고, 빠진 도메인은 리포트의 `regress_overridden`에 남겨 소유자가 사후 검토한다.

→ **정독분에는 마커를 넣고, 자동추출·부분검토분에는 넣지 마라.** 이미 정독 완료한 파일(누적분)에도
   소급으로 이 필드를 추가해 재저장 대상이 되게 하라.

## 표적 우선순위 (소유자 결정: 표적 정독 + 수렴 중단, 2026-07-28)
733 전량을 무차별 정독하지 말고, **자동추출 결함 신호가 강한 문서를 우선** 정독하라(조율자가 목록 제공):
1. **매수인 진술(RW.BUYER) 누락 의심 대형 SPA** — 자동 결과에 매수인 진술이 통째로 빠진 문서.
2. **RW 항목이 비정상적으로 적은 문서**(<5) — 과소추출 잔존.
3. **손배(REM)·확약(COV) 어투가 RW로 잘못 들어간 문서** — 오분류 정정 필요.
나머지 자동추출 결과는 이미 DB에 반영돼 있으니, 조율자가 정독 묶음마다 Gate B를 재측정해
특약계열(90%대)로 **수렴하면 정독을 중단**한다.
