...
개선임을 명시. **#8 text 동의어 확장**(term_dict/alias OR): #2로 gap 재고 (a)(b) 분리 후 착수. **#9 (a)하이브리드
합집합 실측**(eval_absolute_recall 경로 구현됨, 실행만), **(b)** 현 프로브는 `show_duplicates=True`(비관적 모집단)
기준 — 사용자 기본값 dedup은 모집단이 작아 순위가 같거나 낫다. **#10 문서 다양화**(첫 페이지 문서당 item 상한):
정렬 의미 변경이라 최후순위. nDCG@10 관련도 등급 부여는 그 다음이다.
