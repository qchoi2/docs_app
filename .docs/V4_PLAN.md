...
3. **경계 자기설명 필드 강제**: REP vs COV 등 인접분류 item에 "왜 확약이고 진술이 아닌가"를 짧게 적게 하고(qualifier_json 확장), 비거나 상투적이면 감사기가 review 강등. 판단 근거 언어화로 얼버무린 분류 감소.
...
**§9.1 반영(소유자 승인 대기)**: RW 해제 조건에 "**존재형·비교형 정밀도 ≥90%**(family별 표본, 사람 검증)"를 **추가 제안**. §9.1은 소유자 90% 승인으로 고정됐으므로, 이 항목 추가는 소유자 재승인 후 확정한다(추가 시 게이트가 더 엄격해지는 방향).
...
- **RW 부재 게이트 해제 임계치 — 결정됨(2026-07-29): 90% 확정**(§9.1). 이후 항목 추가·임계치 변경 시에만 재승인 필요
