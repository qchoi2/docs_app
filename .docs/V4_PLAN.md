...
recall@3 **+4.5pp**. 아래 #1(토큰화) 반영 후에도 회귀 없음(recall@10 0.905→0.927). 신규 단위 테스트
5개(랭킹·열거순서·다중키워드 AND·low_query_signal·질의로그), 전체 회귀 **487 pass**.

**질의 신호의 3단 구도 (2026-07-30, Fable 2차 검토 반영).** "잔여는 T4"라는 앞의 서술은 **부정확**했다.
T4(임베딩)는 개념-라벨 벤치마크를 못 올린다 — "경업금지" 한 단어의 임베딩은 형제 2,894개와 near-등거리라
같은 벤치마크는 T4 후에도 처참하다. T4가 실제로 이기는 건 **패러프레이즈**(사용자가 조항을 자기 말로 서술)다.
그래서 `eval_ranking_signal.py`에 `--mode paraphrase`를 신설했다(정답의 proposition 어휘로 질의 → verbatim item
탐색; 220건, seed 17):

| 질의 유형 | recall@1 | recall@10 | verbatim 토큰 커버리지 |
|---|---:|---:|---:|
| 개념 라벨만 | ~0 | **1.5%** | — (모집단 한계, 랭킹·T4 모두 못 풂) |
| **패러프레이즈**(조항 서술) | 0.391 | **0.596** | **0.415** ← T4 표적·사전등록 베이스라인 |
| verbatim(조항 인용) | 0.627 | **0.927** | 1.0 (어휘로 거의 해결) |

커버리지 0.415 = 패러프레이즈 질의어의 **59%가 실제 조항 원문에 없다**. **V4-7 ablation은 이 패러프레이즈
recall@10 0.596을 이겨야 채택**된다(게이트: "측정된 기여도 없으면 폐기"의 측정 수단). miss를 분류하면 순수
의미 gap만이 아니다: **(a) 교차언어**(영문 proposition ↔ 국문 verbatim, cov 0) — 사전(양방향)·임베딩,
**(b) boilerplate 공통어**("매도인은…계약을", cov 1.0인데 rank 124) — **IDF/희소도 가중**의 어휘적 여지,
**(c) 퇴화 proposition** — eval artifact. 즉 0.596은 순수 의미 gap을 **과대평가**하며, T4에 귀속 전에 (b)(a-사전)로
닫히는 부분을 먼저 분리해야 한다(§8 사전 확장 → 그 뒤 T4).

**에이전트-대면 개선 (질의에 신호를 싣게 만든다 — 랭커 수정보다 쌈).**
- **#1 다중 키워드 text**: `text`를 공백 토큰화 → 각 토큰이 verbatim/proposition에 존재(AND). 종전 단일 substring은
  "손해배상 상한 10%"류 키워드 질의를 통째 인접 매치 없으면 0건 반환했다. 순수 읽기 경로.
- **#5 MCP docstring**: `text`가 필터가 아니라 **랭킹 신호**임을 명시 — 개념 라벨만 주지 말고 사용자 질의의 변별
  키워드를 항상 text로 싣게 안내(코드 무변경, 회귀 0).
- **#6 `low_query_signal`**: 좁히는 신호 없이 모집단 > `LOW_SIGNAL_POPULATION`(200)이면 결과에 기계가 읽는 힌트를
  넣어 에이전트가 스스로 재질의하게 한다.
- **#7 `v4_query_log.jsonl`**: 실사용 질의(taxonomy_id·has_text·population·narrowed_by)를 파일 append로 기록
  (진입점에서만, eval/probe 제외). "실질의 중 몇 %가 개념-only인가" = T4가 풀 문제의 실제 크기를 합성 벤치마크가
  아닌 실데이터로 판정한다. DB 미기록 → 단일 writer 무관.

**후순위(근거와 함께 보류).** **#4 개념 질의 정적 prior**(라벨-literal/체결본/current/confidence): recall 불변·
브라우징(nDCG)만 개선인데 **모든 구조화 질의의 전역 열거 순서**를 바꾸므로 마무리 시점엔 보류 — 넣을 땐 nDCG형
개선임을 명시. **#10 문서 다양화**(첫 페이지 문서당 item 상한): 정렬 의미 변경이라 최후순위. nDCG@10 관련도 등급
부여는 그 다음이다.

### 9.6-b FTS+bm25 전환 · 공정 베이스라인 확정 (2026-07-30, Fable 3·4차)

**설계-구현 드리프트 최대 사례 — v4_item_fts가 배선돼 있지 않았다.** 스키마에 FTS5 인덱스(proposition+verbatim,
trigram, content-linked)가 있고 트리거 3개로 유지되며 **137,865=137,865 완전 동기화**인데, 검색 코드는 이를 한 줄도
안 쓰고 `text` 필터는 LIKE 전체 스캔이었다(§4 설계의 "2번째 경로"가 미구현). 매 store마다 쓰기 비용만 내고 조회 혜택 0.
→ **`search_clause_items`의 text 경로를 FTS로 전환**: ≥3자 토큰은 `v4_item_fts MATCH`(trigram 인덱스)로 필터, <3자 토큰
(2자 한국어 "상한" 등 trigram 불가)은 LIKE 폴백. 정렬은 **하이브리드** — 연속 전체구문 verbatim 매치 우선(인용 질의
보호) → `bm25()`(관련도+IDF, boilerplate 공통어 down-weight) → 길이. AND 의미·결과셋은 동일, 정렬·검색만 개선.

**bm25 단독은 인용 경로를 회귀시킨다(중요).** 순정 bm25 정렬은 verbatim(정확 인용) recall@1 0.63→0.44로 떨어졌다 —
...
**방식 메모(Fable).** 이 프로젝트 결함은 대부분 코드 버그가 아니라 **설계-구현 사이 조용한 틈**(T4 문서 모순, full_read
마커 누락, FTS 미배선)에서 난다. §4 경로별 "설계된 테이블/경로가 실제 참조되는가" 경량 배선 감사를 주기적으로 돈다.
