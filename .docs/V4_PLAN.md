...
- **T-D taxonomy 후보 backlog 결정(별도 축, 날짜 고정)**: `v4_taxonomy_candidate` pending **29,807**(approved 31·merged 1,540·rejected 16, 2026-07-29 확인). 이 backlog가 "complete인데 후보 미처리"로 다수 문서(2026-07-27 기준 약 43%)의 부재 질의를 막는다 — **재추출 깊이 문제와 별개 축**이라 재추출이 다 끝나도 방치되면 부재 커버리지는 낮게 남는다. 완화안(DEF 문서-특정 과다생성 후보의 생성/제외 기준 교정)은 **소유자 결정 사안**(§11). **결정 시점: PAY tier1 store 완료 시**(다음 마일스톤)로 못박아 방치를 방지.
...
- **RW 부재 게이트 해제 임계치 승인(§9.1 고정 체크리스트)** — 항목 추가·임계치 변경 시 재승인 필수
- **taxonomy 후보 backlog(29,807 pending) 처리 방침 결정**(DEF 문서-특정 과다생성 후보의 생성/제외 기준) — 결정 시점 PAY tier1 store 완료 시(§9.2 T-D)
