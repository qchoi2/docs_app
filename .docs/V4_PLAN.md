...
**완료 기준(소유자 승인 필요 — §11)**: 부재형·존재형과 같은 **셀 정확도 ≥90%**를 권장한다.
근거: 다른 두 축이 90%로 고정돼 있는데 비교형만 낮추면 "필수" 분류와 모순된다. 다만 비교형은
셀 수가 많아 표본 비용이 크므로, 소유자가 표본 규모(질의 6개 전수 vs 일부)를 함께 결정한다.
...
- **비교형 셀 정확도 완료 기준 — 미결정(2026-07-29 제기, §9.4)**: 권장 ≥90%(다른 두 축과 동일).
  표본 규모(V4C01~06 전수 vs 일부)도 함께 결정 필요. §0에서 "필수"로 분류한 기능인데 정확도
  검증 계획이 없던 공백이다.
- **DEF 구조화 검색을 완료 범위에 넣을지 — 미결정(2026-07-29 제기, §3)**: DEF item의 78%
...
- **절대 recall 하한을 §9.1 체크리스트에 넣을지 — 미결정(2026-07-29 제기, §9.5)**: 현재 체크리스트는
  정밀도만 게이팅한다. `eval_absolute_recall.py` 수치가 안정된 뒤 결정.
- **T4를 V4-7 예약 실험으로 편입 — 미결정(2026-07-29 제기, §10.1)**: 품질 최우선 전환에 따른
  재평가. 채택 여부는 골든셋 ablation 결과로만 판정.
- **스캔 PDF 로컬 OCR 도입 여부 — 미결정(2026-07-29 제기)**: 새 외부 의존성(tesseract + 한국어
  데이터) 설치를 수반할 수 있다. 근거는 `.docs/CORPUS_BLIND_SPOTS_20260729.md`.
