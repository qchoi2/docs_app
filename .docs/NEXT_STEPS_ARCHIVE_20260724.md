...
> ## ⚠️ 이 문서는 아카이브(역사 기록)다 — 현재의 "다음 할 일"이 아니다
>
> - 이름과 달리 이 파일은 **2026-07-11 작성 / 2026-07-24 최종 갱신된 39KB 역사 문서**로,
>   T3·V4 착수 전의 실행안과 그 당시 판단 근거를 보관한다.
> - **지금 무엇을 하는가 / 다음은 무엇인가는 [NOW.md](../NOW.md)가 단일 원천이다.**
>   아래 본문의 순서·우선순위는 현재 계획과 다르며, 그대로 따르면 안 된다.
> - 현재 계획의 근거는 [V4_PLAN.md](V4_PLAN.md),
>   [PLAN_REVIEW_20260727.md](PLAN_REVIEW_20260727.md), 이력은 [progress.md](../progress.md)에 있다.
> - 2026-07-29에 루트 `NEXT_STEPS.md`에서 이 경로로 옮겼다(이름이 계속 권위 있는 문서로 오독됨).
> - 본문은 참고용으로 보존한다(삭제하지 않음).

_작성: 2026-07-11. 기준 문서: `docs_progress_v2.md`(티어 사다리), `IMPLEMENTATION_BRIEF.md`,
`NOTES_FOR_OWNER.md`, `PILOT_REPORT_20260710.md`._

> **2026-07-24 정정:** 아래 본문은 T3/V4 착수 전의 역사적 실행안이다.
> 현재는 T3 완료, V4-2 대표 10건 승인·적재 상태이며 최신 순서는
> `V4_PLAN.md`의 V4-3→V4-6을 먼저 완료한 뒤 T4를 구축하는 것이다.
> T4는 승인된 V4 원자 항목을 1차 임베딩 단위로 사용하고 T3 조항·T2 문단을
> 폴백으로 사용한다. 문서와 질문 임베딩, 벡터 검색, reranker는 동일한 로컬
> 모델 계열로 수행하며 외부 API/MCP AI는 최종 답변 생성에만 선택적으로 사용한다.
> 런타임에는 모델 상주, 기본 결과 우선 반환, 질의별 T4 라우팅, 캐시,
> timeout/fallback 및 latency 성능 게이트를 적용한다.
...
V4-5 검색·비교 게이트가 나온 뒤 본격 착수합니다. 승인된 V4 원자 항목을 우선하고,
V4 미평가 범위는 T3 조항과 T2 문단으로 보완합니다. `embed_contracts.py` /
`hybrid_search.py`로 동일한 로컬 모델을 사용해 문서·질의를 임베딩하고, 모델을
검색 프로세스에 상주시킨 뒤 키워드·V4·벡터 결과를 결합합니다. 임베딩 +
재순위를 붙이되, **각 구성요소(벡터, 리랭커)를 골든셋 절제실험(ablation)으로 기여도를 측정해
개별 채택/폐기**합니다. "구현하되 맹신하지 않는다"는 원칙 — 측정으로 도움이 확인된 것만 남깁니다.

정확한 메타·수치·taxonomy 질의에는 기본 검색을 우선하고 의미 질의에만 T4를 실행합니다.
기본 결과는 먼저 보여주며, 재순위가 시간 예산을 넘으면 RRF 결과로 폴백합니다.
외부 API 또는 MCP AI 클라이언트는 최종 후보를 설명·비교하는 선택 경로이고,
질문 임베딩을 다른 외부 모델로 대신 생성하지 않습니다.
