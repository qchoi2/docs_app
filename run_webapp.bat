@echo off
REM ============================================================
REM 계약서 검색 웹앱 실행 (더블클릭용)
REM 이 파일은 한국어 Windows cmd 기본 코드페이지(cp949)로 저장되어 있다.
REM UTF-8로 다시 저장하면 더블클릭 실행이 깨지므로 인코딩을 바꾸지 말 것.
REM 어느 폴더에서 실행해도 이 파일이 있는 프로젝트 폴더로 이동한 뒤
